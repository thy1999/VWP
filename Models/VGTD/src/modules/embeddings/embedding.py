...
class Embedding(torch.nn.Module):
    ...
    def forward(self, x):
        word_emb = [item.emb(x[item.field]) for item in self.embeds]
        if self.cat_output:
            word_emb = torch.cat(word_emb, dim=-1)
        
        if ('img_path_list' in x.keys()):
            image_emb = self.img_embedding(x['img_path_list'].tolist())
