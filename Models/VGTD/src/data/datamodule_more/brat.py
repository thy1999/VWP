...
class BratDoc:
    ...
    def to_word(self):
        ...
        return BratDoc(self.id, words, self.pic, new_spans, new_span_pairs, bracket_file=self.bracket_file, tree=self.tree)

    def split_pic_for_every_sentence(self, pic):
        pics_list = []
        for one_sentence_pics in pic.split('\n'):
            pics_list.append(one_sentence_pics.split("\t"))
        return pics_list
    ...
    def split_by_sentence(self, sentencizer=None) -> List['BratDoc']:
        ...
        pics = self.split_pic_for_every_sentence(self.pic)
        # split bracket file
        ...
        for i, spans in enumerate(spans_per_sent):
            ...
            brat_doc_li.append(BratDoc(self.id, sents[i][0], pics[i], dict(spans), span_pair, tree=tree))
    ...
    @classmethod
    def from_file(cls, text_file: str, ann_file: str, pic_file: str, bracket_file: str = None):
        """ read text and annotations from files in BRAT format """
        with open(text_file, 'r') as txtf:
            doc = txtf.read().rstrip()
        with open(pic_file, 'r') as picf:
            pic = picf.read().rstrip()
        spans = {}
        span_pairs = {}
        ...
        return cls(ann_file, doc, pic, spans, span_pairs_converted, bracket_file=bracket_file)
...
class _BratLoader(Loader):
    ...
    def _load(self, root_dir):
        ...
        for brat_doc in self.walk(root_dir):
            ...
            inst = {'raw_words': brat_doc.doc, 'raw_pics': brat_doc.pic, 'raw_span': brat_doc.spans, 'raw_span_pair': brat_doc.span_pairs}
            if brat_doc.tree:
    ...
    def walk(self, root_dir):
        for root, dirs, files in os.walk(root_dir, followlinks=True):
            for file in files:
                ...
                pic_file = os.path.join(root, file[:-4] + '.pic')
                ...
                if not os.path.exists(pic_file):
                    log.warning(f'No matching .pic file for {pic_file}')
                    continue 
                if not os.path.exists(bracket_file):
                    bracket_file = None
                brat_doc = BratDoc.from_file(text_file, ann_file, pic_file, bracket_file=bracket_file)
                yield from brat_doc.split_by_sentence(sentencizer=self.sentencizer)


class BratNER(DataModule):
    INPUTS = ('id', 'words', 'seq_len', 'img_path_list')
