...
def output(infn, docnum, sentences):
    ...
    if output_directory is None:
        ...
        picout = codecs.open(outfn + '.pic', 'w', encoding=OUTPUT_ENCODING)

    offset, idnum, ridnum = 0, 1, 1
    doctext = '\n'.join(' '.join(tokens) for tokens, _, _ in sentences)
    print(doctext, file=txtout)
    pictext = '\n'.join('\t'.join(pics) for _, _, pics in sentences)
    print(pictext, file=picout)

    for si, sentence in enumerate(sentences):
        tokens, spans, pics = sentence
