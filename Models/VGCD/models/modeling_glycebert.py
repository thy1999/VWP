...
from models import transformer_attention
...
class GlyceBertModel(BertModel):
    ...
    def forward(
        ...
        if batch_images_list:
            text_embeddings, image_embeddings, img_len_list = self.embeddings(
                input_ids=input_ids, pinyin_ids=pinyin_ids, batch_images_list=batch_images_list, position_ids=position_ids, token_type_ids=token_type_ids,
            ...
            attention_mask = torch.zeros(bs, max_seq_len, max_img_len).to(text_embeddings.device)
            # 填充掩码的有效部分为 1
            for i, (seq_len, image_num) in enumerate(zip(seq_len_list, img_len_list)):
                attention_mask[i, :seq_len, :image_num] = 1
            
            embedding_output = text_embeddings
            # 对embedding的结果进行encode。
            ...
            cross_output = self.attention(sequence_output, image_embeddings, image_embeddings, attention_mask)
            if self.fusion_way == "WeightedSum":
                sequence_output = self.beta * sequence_output + (1.0 - self.beta) * cross_output
            else:
                sequence_output = cross_output

            # cross_embed = self.attention(text_embeddings, image_embeddings, image_embeddings, attention_mask)
            # if self.fusion_way == "WeightedSum":
            #     embedding_output = self.beta * text_embeddings + (1.0 - self.beta) * cross_embed
            # else:
            #     embedding_output = cross_embed
        else:
